...
usage = """%s [options] input patch hold""" % prog
...
# Get data only for stars and select the extinction for the hold filter.
...
#Check number of sources in patchs
patchs = set(star['tract'])
print patchs
for patch in patchs:
	print patch, len(star[star['tract'] == patch])
print len(star)
...
mags_err_hold = {f: star_p[star_p['filter'] == f]['modelfit_CModel_mag_extcorrSigma'] for f in filters}
...
thdulist.writeto('mag_' + args.patch + '.fits')

# Check fits file content
hdu_list = fits.open('mag_' + args.patch + '.fits')
hdu_list.info()

# Check first extension
g = getdata('mag_' + args.patch + '.fits', 1)
t = Table(g)
# print t.more()
